...
import { _global, getFlag, setFlag, nativeTryCatch } from '@brolisten/utils';
import { SDK_VERSION, SDK_NAME, EVENTTYPES } from '@brolisten/common';
import { InitOptions, VueInstance, ViewModel } from '@brolisten/types';

function init(options: InitOptions) {
  if (!options.dsn || !options.apikey) {
    return console.error(`browser-listen 缺少必须配置项：${!options.dsn ? 'dsn' : 'apikey'} `);
  }
