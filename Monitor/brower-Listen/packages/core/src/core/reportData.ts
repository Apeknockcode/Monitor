import {
  ...
  getLocationHref,
  generateUUID,
} from '@brolisten/utils';
import { SDK_VERSION, EVENTTYPES } from '@brolisten/common';
import { ReportData, InitOptions } from '@brolisten/types';
...
export class TransportData {
  ...
  getAuthId(): string | number {
    if (typeof this.getUserId === 'function') {
      ...
      if (typeof id === 'string' || typeof id === 'number') {
        ...
        console.error(
          `browser-listen userId: ${id} 期望 string 或 number 类型，但是传入 ${typeof id}`
        );
      }
    }
    return '';
  ...
  async send(data: ReportData) {
    ...
    if (isEmpty(dsn)) {
      console.error('browser-listen: dsn为空，没有传入监控错误上报的dsn地址，请在init中传入');
      return;
    }
    // 开启录屏，由@brolisten/recordScreen 插件控制
