import { getFlag, nativeTryCatch, setFlag } from '@brolisten/utils';
import { ReplaceHandler, ReplaceCallback } from '@brolisten/types';
...
export function notify(type: EVENTTYPES, data?: any): void {
  ...
  handlers[type]?.forEach(callback => {
    nativeTryCatch(
      ...
      () => {
        // console.error(
        //   `browser-listen 重写事件notify的回调函数发生错误\nType:${type}\nName: ${getFunctionName(
        //     callback
        //   )}\nError: ${e}`
        // );
      }
